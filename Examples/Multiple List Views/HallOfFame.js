(function(){
	...
	function hofItem(ctx){
		...
		if(!ctx.csrShim && ctx.CurrentItem.Person[0].picture){
			photoUrl = ctx.CurrentItem.Person[0].picture;
		} else if(ctx.csrShim && ctx.CurrentItem["Person.picture"]){
			photoUrl = ctx.CurrentItem["Person.picture"];
		}
	    var photo = '/_layouts/15/userphoto.aspx?accountname=' + sip + '&size=L&url=' + photoUrl;
	    var userUrl = _spPageContextInfo.siteServerRelativeUrl + '/_layouts/15/userdisp.aspx?ID=' + userId; 		    				
